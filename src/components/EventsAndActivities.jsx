...
const EventsAndActivities = () => {
  ...
  // Master defined filter list to keep categories grouped cleanly
  const categoryOrder = [
    'All',
    'Industry Connect',
    ...
    'Podcasts & Media',
    'Orientations & Campus Events'
  ];

  const presentTags = new Set(allEvents.map(e => e.tag));
  const availableTags = categoryOrder.filter(c => c === 'All' || presentTags.has(c));
            ...
            {availableTags.map(tag => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-bold transition-all duration-200 ${
                  selectedTag === tag
                    ? 'bg-sparkBlue text-white shadow-md ring-2 ring-sparkAccent/50'
